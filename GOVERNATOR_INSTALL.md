...
2. `fleet/godot_controller.py chairs` loads `telemetry/controller.py` and uses
   its validated `SensorSource` occupancy model directly.
3. Occupied chairs become the absolute active Godot regime set on UDP 5005.
4. The optional diagnostic publisher uses UDP 5006, so it cannot collide with
