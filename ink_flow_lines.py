...
DRAINS = [
    #Drain(10.3, 3.5, attraction=2.0, swirl=5.8, capture_radius=0.20),
]
...
def spawn_positions(count: int) -> np.ndarray:
    """
    Spawn particles along the left edge.

    The slight horizontal spread prevents all trails from starting on
    exactly the same vertical line.
    """
    return np.column_stack(
        (
            rng.uniform(-0.15, 0.25, count),
...
rng = int(random.Random(SEED))
trails = np.repeat(positions[:, None, :], rng
, axis=1)
