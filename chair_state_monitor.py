...
Each channel is labeled with the regime controlled by that chair.
...
# Godot owns UDP 5005. Chair diagnostics are published separately on 5006.
UDP_PORT = 5006
HISTORY_S = 60.0
...
def render_plain(st):
    ...
    lines = [f"  SACRAMENTO MODEL   source: {st.get('source', '?'):<9}"
             ...
             f"   {'channel / regime':<26} {'state':<10} {'temp':>7}  {'vote':>6}"]
    for i, c in enumerate(chairs):
        n = i + 1
        state = "DORMANT" if n in stale else ("ACTIVATED" if c else "free")
        t = temps[i] if i < len(temps) else None
        v = votes[i] if i < len(votes) else 0.0
        ts = f"{t:.1f}C" if t is not None else "-"
        bar = "█" * int(round((v or 0) * 12))
        label = f"{n}  {REGIME_NAMES[i]}"
        lines.append(f"   {label:<26} {state:<10} {ts:>7}  "
                     f"{v:>6.2f} {bar}")
...
def run_graphical(port):
    ...
    for i in range(7):
        ...
        fillref = [None]
        name = ax.text(0.008, 0.80, f"{i+1}  {REGIME_NAMES[i]}",
    ...
    def update(_):
        ...
        for i, r in enumerate(rows):
            ...
            r["state"].set_text("DORMANT" if off else
                                ("ACTIVATED" if occ else "free"))
    ...
    def close_monitor(event):
        if event.key in {"escape", "ctrl+c"}:
            plt.close(fig)

    fig.canvas.mpl_connect("key_press_event", close_monitor)
    try:
        plt.show()
