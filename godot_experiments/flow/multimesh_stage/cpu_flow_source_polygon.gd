class_name CPUFlowSourcePolygon
extends Resource

## Runtime-addressable source geometry for CPU-owned water heads.
##
## Vertices and flow_direction deliberately have no screen/world conversion.
## They must use the same coordinate system and units; the stage owns any
## conversion. emission_fraction and enabled are policy fields for the caller:
## sampling itself is a pure geometry operation and does not suppress points.
...
## NO_SEED means that sample01 is used directly. A nonnegative seed applies a
## stable phase to sample01 without introducing mutable random state.
...
func validate() -> String:
	## Return an empty String when all directly exported fields form a source.
...
func sample_emission_point(
	...
	## Map sample01 deterministically over the downstream-facing boundary.
	## downstream_epsilon uses the same units as vertices and nudges outward.
	if (
	...
	var edges: Array[Dictionary] = selected_downstream_edges(
		vertices,
		flow_direction,
	)
	if edges.is_empty():
	...
	if seed != NO_SEED:
		var phase := FlowMath.stable_unit_sample(0, seed)
		unit_sample = FlowMath.unit_fraction(unit_sample + phase)
	...
	for edge_array_index in range(edges.size()):
		...
		if target_weight < accumulated_weight + edge_weight or is_last_edge:
			...
			return (
				start.lerp(end, local_sample)
				+ outward_normal * downstream_epsilon
			)
...
static func selected_downstream_edges(
	...
	## Return downstream-facing edges in polygon order for sampling/debugging.
	## Each edge carries edge_index, start, end, outward_normal, length,
	## alignment, and weight. weight = length * alignment.
...
static func _validation_error_for(
	...
	if candidate_element_id == &"":
		return "A source polygon needs a nonempty element_id."
	if not is_valid_polygon(candidate_vertices):
		return "A source polygon needs 3-12 finite vertices forming a simple polygon."
	if (
